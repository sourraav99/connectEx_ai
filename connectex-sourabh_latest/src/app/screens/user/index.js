...
const Users = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      dispatch(getSelfProfileAction((res) => {
        setprofileData(res?.data?.profile);
      }));
    }, [dispatch]) // Include dispatch as dependency
  );

  // console.log(`${IMAGE_BASE_URL}/public/profileImage/${profileData.basicInfo.image}}`);

  const navigateToSetupProfile = () => {
    navigation.navigate('ProfileSetup', { profileData: profileData })
  }

  const Dots = ({ numDots, dotSize, dotColor, style }) => {
    return (
        ...
        {Array.from({ length: numDots }).map((_, index) => (
          <View
            ...
            style={{
              ...
              backgroundColor: dotColor, // Dot color
              borderRadius: dotSize / 2, // Make it circular
              marginHorizontal: dotSize / 10, // Spacing between dots
            }}
          />
        ))}
      </View>
  ...
  return (
    <ImageBackground source={IMAGES.appBackground} style={{ flex: 1 }}>
      <View style={{ flex: 1, backgroundColor: COLORS.BLACK_OPACITY(0.38) }}>
        ...
        <View style={{ flexDirection: 'row', width: '98%', alignSelf: 'center', alignItems: 'center', marginTop: verticalScale(10), paddingBottom: 12 }}>
          <Image
            resizeMode='cover'
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1, flexGrow: 1 }}>
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('Settings')} hitSlop={10} style={{ marginHorizontal: moderateScale(8), height: verticalScale(20), width: verticalScale(20), alignItems: 'center', justifyContent: 'center' }}>
                <Ionicons name={'settings'} color={COLORS.WHITE_OPACITY(0.8)} size={scale(18)} />
              </TouchableOpacity>
            </View>
          </View>
          <View style={{ width: "98%", backgroundColor: COLORS.WHITE_OPACITY(0.3), alignSelf: 'center', borderRadius: scale(10), marginTop: verticalScale(40) }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: moderateScale(10), marginTop: verticalScale(10) }}>
              <View style={{ flexDirection: "row", alignItems: 'center' }}>
                ...
                <Text style={{ color: COLORS.WHITE, fontSize: scale(15), fontWeight: '500' }}>4.9</Text>
              </View>
              <View style={{ position: 'absolute', alignItems: 'center', width: '98%', top: verticalScale(-35), marginLeft: moderateScale(12) }}>
                ...
                <View style={{ height: verticalScale(65), width: verticalScale(65), borderRadius: 100, borderWidth: 3, borderColor: COLORS.WHITE, alignItems: 'center', justifyContent: 'center' }}>
                  <Image resizeMode='cover' style={{ height: verticalScale(59), width: verticalScale(59), borderRadius: 100, }} source={{
                    uri: profileData?.basicInfo?.image?.trim() ? `${IMAGE_BASE_URL}/${profileData.basicInfo.image}` : 'https://i.pinimg.com/736x/8b/16/7a/8b167af653c2399dd93b952a48740620.jpg',
                  }} />

                </View>
                <Text style={{ color: 'white', fontSize: scale(14) }}>{`${profileData?.basicInfo?.name || 'name'}`}</Text>
              </View>
              <TouchableOpacity onPress={navigateToSetupProfile} style={{ height: verticalScale(20), width: verticalScale(20), borderRadius: 100, backgroundColor: COLORS.WHITE, alignItems: 'center', justifyContent: 'center' }}>
                <FontAwesome6 name="pen" size={14} color={COLORS.app_violete} />
              </TouchableOpacity>
            </View>
            <View style={{ marginTop: verticalScale(30), alignSelf: 'center' }}>
              <Text style={{ color: 'white', fontSize: scale(10),textAlign:'center' }}>{`${profileData?.aboutMe?.describeYourself || ''}`}</Text>
            </View>
            <View
              style={{
                flexDirection: 'row',
            ...
            >
              ...
              <Dots
                numDots={30} // Number of dots
                dotSize={screenWidth / (numDots * 1.7)} // Size of each dot
                dotColor="grey" // Color of the dots
              // style={{ marginVertical: 10 }} // Additional styling
              />
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: moderateScale(14), paddingVertical: verticalScale(5) }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Text style={{ color: COLORS.WHITE, fontSize: scale(11), fontWeight: '500', marginLeft: 5 }}>{profileData?.aboutMe?.experienceInYear ? `${profileData.aboutMe.experienceInYear} Years+` : 'Add experience'}</Text>
              </View>
              <TouchableOpacity onPress={navigateToSetupProfile} activeOpacity={0.7} style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Text style={{ fontSize: scale(11), color: COLORS.WHITE }}>Add your expertise</Text>
            ...
            <TouchableOpacity activeOpacity={1} onPress={navigateToSetupProfile} style={{ flexDirection: 'row', marginVertical: verticalScale(7), alignItems: 'center', paddingHorizontal: moderateScale(7) }}>
              <TouchableOpacity style={{
                ...
                <Text style={{ marginLeft: 2, fontSize: scale(10), color: COLORS.BLACK }}>{`₹${profileData?.priceDetails?.chatCall || 50}`}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{
                flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.WHITE,
                color: COLORS.BLACK,
                ...
                <Text style={{ marginLeft: 2, fontSize: scale(10), color: COLORS.BLACK }}>{`₹${profileData?.priceDetails?.audioCall || '200'}`}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{
                flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.WHITE,
                color: COLORS.BLACK,
                ...
                <Text style={{ marginLeft: 2, fontSize: scale(10), color: COLORS.BLACK }}>{`₹${profileData?.priceDetails?.videoCall || '500'}`}</Text>
              </TouchableOpacity>
              <View style={{ flexDirection: 'row', position: 'absolute', right: moderateScale(17) }}>
                <Text style={{ fontSize: scale(11), color: COLORS.WHITE }}>Update</Text>
                <AntDesign name="arrowright" size={15} color={COLORS.GREY} />
              </View>
            </TouchableOpacity>
          </View>

          <View style={{ flexDirection: 'row', width: "97%", backgroundColor: COLORS.WHITE_OPACITY(0.3), paddingHorizontal: moderateScale(15), paddingVertical: verticalScale(10), margin: scale(5), alignSelf: 'center', borderRadius: scale(10), alignItems: 'center', marginVertical: verticalScale(7) }}>
            <View style={{ backgroundColor: COLORS.WHITE, height: verticalScale(35), width: verticalScale(35), borderRadius: 100, alignItems: 'center', justifyContent: 'center' }}>
              <EvilIcons name="calendar" size={scale(18)} color={COLORS.app_violete} />
            </View>
            <View style={{ marginLeft: moderateScale(16), }}>
              <Text style={{ fontSize: scale(12), color: COLORS.WHITE }}>All Day Availability</Text>
              <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', }}>
                <Text style={{ fontSize: scale(10), color: COLORS.WHITE_OPACITY(0.5) }}>or Set Schedule</Text>
                <AntDesign name="arrowright" size={15} color={COLORS.WHITE} />
              </TouchableOpacity>
            </View>
            <Switch
              style={{ marginLeft: moderateScale(110) }}
              trackColor={{ false: COLORS.GREY_OPACITY(0.4), true: COLORS.app_violete }}
          ...
          <View
            style={{
              flexDirection: 'row', flexWrap: 'wrap', alignSelf: 'center', width: '105%', alignItems: 'center', justifyContent: 'center'
            }}
          >
            {iconArray.map((icon) => {
              ...
              return (
                ...
                >
                  <View style={{ backgroundColor: COLORS.WHITE_OPACITY(0.7), height: verticalScale(35), width: verticalScale(35), alignItems: 'center', justifyContent: 'center', borderRadius: 100 }}>
          ...
          <TouchableOpacity onPress={navigateToSetupProfile} activeOpacity={0.7} style={{ width: "98%", backgroundColor: COLORS.WHITE_OPACITY(0.3), borderRadius: scale(10), alignSelf: 'center', padding: scale(10), marginTop: verticalScale(5) }}>
